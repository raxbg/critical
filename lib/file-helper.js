...
function replaceAssetPaths(html, opts) {
    ...
    return function (stylesheet) {
        ...
        const css = stylesheet.contents.toString().replace(/url\(['"]?([^'"\\)]+)['"]?\)/g, (match, assetPath) => {
            ...
            let stylesheetPath;
            let assetRelative;
            const remoteUrl = fileToUrl[path.resolve(stylesheet.path)];
            // Create asset path relative to opts.base
            if (remoteUrl && !path.basename(remoteUrl).match(/\.html$/)) {
              stylesheetPath = path.dirname(remoteUrl);
              assetRelative = path.relative(process.cwd(), path.resolve(path.join(stylesheetPath, assetPath)));
            } else {
              stylesheetPath = path.dirname(stylesheet.path);
              assetRelative = path.relative(baseResolved, path.resolve(path.join(stylesheetPath, assetPath)));
            }
            ...
            return normalizePath(match.replace(assetPath, path.join(pathPrefix, assetRelative).replace(/^(https?):\/(?!\/)/, "$1://")));
        });
 ...
 * @returns {Promise}
 */
function temp(resp, opts) {
    const urlParts = url.parse(resp.url);
    const pathname = urlParts.pathname;
    const contentType = resp.headers['content-type'];
    return Promise.resolve()
        .then(() => {
            if (!pathname || mime.extension(contentType) == "html") {
                var filepath = tempy.file({extension: mime.extension(contentType)});
            } else {
                var filepath = opts.base + path.dirname(pathname) + path.sep + Buffer.from(path.basename(pathname) + urlParts.searchParams).toString('base64');
            }
            fileToUrl[filepath] = resp.url;
            gc.addFile(filepath);
...
function assertLocal(filePath, opts = {}) {
    ...
    return requestAsync(filePath, true, opts)
        .then(response => temp(response, opts));
}
